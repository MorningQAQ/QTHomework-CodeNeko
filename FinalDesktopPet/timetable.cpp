    ...
    , ui(new Ui::timetable)
{
    ...
    QObject::connect(TableWidget, &QTableWidget::itemClicked, this, [&](QTableWidgetItem* item) {
        ...
        if (Cho) {
            ...
            if (item->text() != "") {
                classname->setText(((myqwitem*)TableWidget->item(latest % 13+1, latest / 13))->Classname);
                teacher->setText(((myqwitem*)TableWidget->item(latest % 13+1, latest / 13))->Teacher);
                classroom->setText(((myqwitem*)TableWidget->item(latest % 13+1, latest / 13))->Classroom);
                rmk->setText(((myqwitem*)TableWidget->item(latest % 13+1, latest / 13))->Rmk);
                C0 = ((myqwitem*)TableWidget->item(latest % 13+1, latest / 13))->realcolor;
                show0();
                choose->click();
            }
            return;
